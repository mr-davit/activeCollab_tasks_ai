...
/**
 * Static Agent Instructions - User Manual for AI Agents
 */
const AGENT_INSTRUCTIONS = `# AC Task: Agent Instructions
...
## 2. Common Patterns
- **Dashboard:** \`ac-task list --format json\` (defaults to my open tasks)
- **All Tasks:** \`ac-task list --all --format json\`
- **Search:** \`ac-task list --search "query" --format json\`
- **Details:** \`ac-task show <task_id> --format json\`
- **Create:** \`ac-task create "Title" --assignee <id_from_guide> --list <id_from_guide> --format json\`
- **Update:** \`ac-task update <task_id> --name "New Title" --due YYYY-MM-DD --format json\`
- **Complete:** \`ac-task update-status <task_id> done\`
- **Reopen:** \`ac-task update-status <task_id> open\`
- **Comment:** \`ac-task comment <task_id> "Comment text" --format json\`
- **Log Time:** \`ac-task log <task_id> <hours> --summary "Work done" --job-type <id> --format json\`
- **Delete:** \`ac-task delete <task_id> --force\` (use with caution)
...
/**
 * Save the static agent instructions file
 */
function saveAgentInstructions(targetDir: string): void {
    ...
    fs.writeFileSync(instructionsPath, AGENT_INSTRUCTIONS, 'utf-8');
}
